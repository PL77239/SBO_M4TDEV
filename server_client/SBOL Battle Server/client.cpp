...
void Client::SendRivalRecords()
{
	...
	// JEDYNA ZMIANA TUTAJ: '3' na '0' na początku tablicy
	uint8_t status_remaps[] = { 0, 0, 1, 2 };
...
void Client::SendRivalJoin()
{
	...
	for (auto& rival : rivals)
	{
		...
		outbuf.append<uint32_t>(0, false); // starting sp?
		...
		outbuf.append<uint8_t>(0x40); // 0x40 aktywuje zieloną strzałkę i status BATTLE OK!
...
void Client::ProcessPacket()
{
	if (!todc)
	{
		// --- OSTATECZNY HACK: PVP MIĘDZY ŻYWYMI GRACZAMI ---
		if (inbuf.getSize() == 8 && inbuf.buffer[2] == 0x04)
		{
			// 1. ZMIANA STATUSU W PAMIĘCI SERWERA
			if (inbuf.buffer[3] == 0x01)
			{
				if (inbuf.buffer[7] == 0x08) status |= PLAYERSTATUS::PS_SAFEMODE;
				else if (inbuf.buffer[7] == 0x00) status &= ~PLAYERSTATUS::PS_SAFEMODE;
			}

			bool pvpForced = false;

			// 2. PRZESZUKIWANIE ŻYWYCH GRACZY NA MAPIE (Brute-force ID 0-1000)
			if (course != nullptr)
			{
				for (uint16_t i = 0; i < 1000; i++)
				{
					Client* otherPlayer = course->getClient(i);

					// Sprawdzamy, czy to istniejący gracz, czy to nie Ty sam i czy nie jest botem
					if (otherPlayer != nullptr && otherPlayer != this && otherPlayer->isClient)
					{
						// A) SYNCHRONIZACJA WIZUALNA
						// Odsyłamy info o Twoich światłach/Safe Mode do znajomego, żeby jego gra to odrysowała!
						otherPlayer->outbuf.clearBuffer();
						otherPlayer->outbuf.setSize(0x06);
						otherPlayer->outbuf.setOffset(0x06);
						otherPlayer->outbuf.setType(inbuf.getType());
						otherPlayer->outbuf.setSubType(inbuf.getSubType());
						otherPlayer->outbuf.append<uint8_t>(inbuf.buffer[4]);
						otherPlayer->outbuf.append<uint8_t>(inbuf.buffer[5]);
						otherPlayer->outbuf.append<uint8_t>(inbuf.buffer[6]);
						otherPlayer->outbuf.append<uint8_t>(inbuf.buffer[7]);
						otherPlayer->Send();

						// B) WYMUSZENIE BATTLE (Reakcja na długie światła)
						if (inbuf.buffer[3] == 0x00 && inbuf.buffer[7] == 0x01 && !pvpForced)
						{
							// Jeśli oboje nie macie Safe Mode i nie walczycie
							if (battle.status == BATTLESTATUS::BS_NOT_IN_BATTLE &&
								otherPlayer->battle.status == BATTLESTATUS::BS_NOT_IN_BATTLE &&
								!(otherPlayer->status & PLAYERSTATUS::PS_SAFEMODE))
							{
								// Łączymy Was w parę i zmuszamy klienty do załadowania UI
								battle.challenger = otherPlayer;

								logger->Log(Logger::LOGTYPE_CLIENT, L"!!! SERWER WYMUSZA PVP: %u atakuje %u !!!", courseID, otherPlayer->courseID);

								SendBattleChallenge(courseID, otherPlayer->courseID);
								SendBattleChallengeToOpponent(courseID, otherPlayer->courseID);
								pvpForced = true; // Zabezpieczenie przed wyzwaniem kilku osób naraz
							}
						}
					}
				}
			...
			// 3. ECHO DO CIEBIE (Potwierdzenie dla Twojej gry)
			outbuf.clearBuffer();
			outbuf.setSize(0x06);
			outbuf.setOffset(0x06);
			outbuf.setType(inbuf.getType());
			outbuf.setSubType(inbuf.getSubType());
			outbuf.append<uint8_t>(inbuf.buffer[4]);
			outbuf.append<uint8_t>(inbuf.buffer[5]);
			outbuf.append<uint8_t>(inbuf.buffer[6]);
			outbuf.append<uint8_t>(inbuf.buffer[7]);
			Send();
		}
		// ------------------------------------------------------------
