...
void ClientPacketCourseDetails(Client* client)
{
	...
	case 0x301:
	{
		...
		// --- nie jest w trybie beginner ---
		client->notBeginner = true;
		...
		client->outbuf.append<uint16_t>(client->courseID);
		client->outbuf.append<uint32_t>(1);

		client->outbuf.append<uint16_t>(client->courseID);
		client->outbuf.append<uint32_t>(1);
		client->outbuf.append<uint16_t>(1); // MUSI BYĆ 1 (żeby nie psuć matrycy)
		//for (int32_t i = 0; i < 0xE4 / 2; i++) client->outbuf.append<uint16_t>(i, false);
		TimeAttack_OnEnter(client, client->currentCourse); // MUSI BYĆ ODKOMENTOWANE
	}
	break;
	case 0x302:
	{	// responding to this crashes client but this appears to specify which shop the client has entered.
		...
		// --- NASZA MODYFIKACJA NR 1: ZAPISANIE TRASY ---
		...
		// --- ODBLOKOWANY PAKIET WEJŚCIA NA MAIN COURSE ---
		...
		{	// Live cam stuff. Only send while entering main course
			...
			//client->Send(); // WAŻNE: Dodajemy natychmiastowe wysłanie tego pakietu
		}

		// --- NASZA MODYFIKACJA NR 2: PRZYZNANIE NAGRODY ---
